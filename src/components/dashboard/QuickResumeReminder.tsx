...
export function QuickResumeReminder() {
  ...
  useEffect(() => {
    (async () => {
      ...
      } catch (e) { console.error('Error fetching last activity:', e); }
  ...
  /* ── Handlers ── */
  const handleResume = () => {
    if (lastActivity?.moduleId) navigate(`/module/${lastActivity.moduleId}`);
    else if (lastActivity?.courseId) navigate('/courses');
  };
